...
| 2026-09-23 | main (post-handoff) | ARCHITECTURE_NOTES 1 evidence/validation | DONE, 776→779 unit tests green | `evidence_status` (EXTRACTED/INFERRED/GENERATED/USER_APPROVED) and `validation_status` (UNVERIFIED/…) are separate fields on every model that carries a judgement; parser output is EXTRACTED+UNVERIFIED, classifier INFERRED with confidence, planner GENERATED until the user chooses a variant; old JSON without the fields still validates. ADR-0006 |
| 2026-09-23 | main (post-handoff) | ARCHITECTURE_NOTES 2 lineage | DONE | every artifact and `job.json` carry `parent_project_id`, `parent_hash`, `operation_set_id`, stage ORIGINAL→PROPOSED→USER_WORKING→EXPORTED; test asserts the ORIGINAL's hash is unchanged after a build. ADR-0006 |
| 2026-09-23 | main (post-handoff) | ARCHITECTURE_NOTES 3 three-layer arrangement | DONE | Layer A Section intent, Layer B PlaylistOp with reason/evidence/confidence/permission/reversibility, Layer C MutationOp in ticks; `compile_plan` is the only route from B to C and the writer consumes only C; `operations.json` records what became of each op. ADR-0007 |
| 2026-09-23 | main (post-handoff) | ARCHITECTURE_NOTES 4 pipeline stages + resume | DONE, verified on fixtures | 12 named stages in `job.json` with input/output hash, analysis_version, configuration_hash, timestamps; **Resume** on the interrupted-job banner reuses parse, analysis and the chosen plan only when all three hashes match, into a new versioned folder; a changed genre invalidates the plan; the two tests for this caught two real defects before commit. **Not yet exercised on an interrupted job on the PC** |
| 2026-09-23 | main (post-handoff) | ARCHITECTURE_NOTES 5 ground-truth fixtures | DONE as synthetic fixtures, real files still owed | A (4-bar loop), B (finished arrangement), B-unfinished built in `tests/fixtures/ground_truth.py`; tests check tempo, roles, repetition and that the planner's structure is within 0.5×–2× of B's length; an AST check proves the finished B is never handed to the planner. `loop_test.flp` and a real finished project are the intended replacements once they parse on the PC |
| 2026-09-23 | main (post-handoff) | Verification script §5 | NOT RUN — needs the PC | nothing in this session can launch FL or read the real files; every row above is fixture- or synthetic-verified only |
| 2026-09-23 | main `429b385` | CI + release | GREEN | `ci` had been red at the Windows-target Rust check on every run since the step existed (`llvm-rc` missing on the runner); fixed; release run 35812690089 rebuilt v0.1.0 from this commit, ZIP checksum verified against SHA256SUMS.txt, build-info names the commit |
